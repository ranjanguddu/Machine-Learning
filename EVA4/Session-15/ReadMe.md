...
In the above folder tracker_file.txt is a file where i have mapped the BG image with the corresponding FG_BG overlayed inage. street_bg folder has all background images. Then we unzip all the zip files present in those folder in a folder name \'Unzipped_DATA'. Then run the below script:
